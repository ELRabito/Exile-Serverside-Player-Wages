...
*
*/
private ["_playerMoney","_amount","_newPlayerMoney","_playerScore","_increase"];
{
	...
	if (alive _player) then
	{
		...
		}else
		{
			...
			_playerMoney = _player getVariable ["ExileMoney", -1];
			if(_playerMoney >= 0)then
			{	
				_playerScore = _player getVariable ["ExileScore",0];
				if(_playerScore < 0)exitWith				
				{
				...
				_increase = round (_playerScore * 0.001); // INCREASE DEPENDING ON THE RESPECT AMOUNT
				if (_increase < 1) then 
				{
					_increase = 0;
				};
				_amount = 250 + _increase; // Poptab Amount
